import { getRecipeById, toggleStar, toggleFavorite, getUserRecipeInteractions, copyRecipe } from './recipeService.js';
...
function updateRecipeUI(recipe) {
    ...
    if (recipeImage) {
        recipeImage.src = recipe.image || '../LOGO/recipe-placeholder.svg';
...
function setupEventListeners(recipeId) {
    ...
    if (starBtn) {
        starBtn.addEventListener('click', async () => {
            ...
            if (starCount) {
                const newCount = parseInt(starCount.textContent) + 1;
                starCount.textContent = newCount;
            }
            Notifications.show('Recipe rated successfully!', 'success');
        });
    }
    ...
    if (favoriteBtn) {
        favoriteBtn.addEventListener('click', async () => {
            ...
            Notifications.show('Recipe added to favorites!', 'success');
        });
    }

    // Save Recipe
    const saveBtn = DOM.get('save-btn');
    if (saveBtn) {
        saveBtn.addEventListener('click', async () => {
            if (!Auth.isAuthenticated()) {
                Notifications.show('Please sign in to save recipes', 'info');
                return;
            }
            await toggleFavorite(recipeId);
            saveBtn.classList.toggle('active');
            Notifications.show('Recipe saved successfully!', 'success');
        });
    }
    ...
    if (copyBtn) {
        copyBtn.addEventListener('click', async () => {
            if (!Auth.isAuthenticated()) {
                Notifications.show('Please sign in to copy recipes', 'info');
                return;
            }
            try {
                await copyRecipe(recipeId);
                Notifications.show('Recipe copied to your collection!', 'success');
            } catch (error) {
                console.error('Error copying recipe:', error);
                Notifications.show('Failed to copy recipe', 'error');
            }
        });
    }

    // Remove share button
    const shareBtn = DOM.get('share-btn');
    if (shareBtn) {
