...
const { checkDuplicateRecipe } = require('../utils/recipeDeduplication');
...
const createRecipe = async (req, res) => {
    try {
        ...
        // Check for duplicates if recipe is public
        if (visibility === 'public') {
            const duplicateCheck = await checkDuplicateRecipe(recipe);
            if (duplicateCheck.isDuplicate) {
                return res.status(400).json({
                    message: 'Similar recipe already exists',
